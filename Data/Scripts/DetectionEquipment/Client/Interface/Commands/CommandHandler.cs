    ...
    public class CommandHandler
    {
        ...
        private readonly Dictionary<string, Command> _commands = new Dictionary<string, Command>
        {
            ...
            ["getallangles"] = new Command(
                "DetEq",
                "Calculates the RCS of the looked at grid for all icosphere angles and prints it out to a storage file \"DetEq_RCS_Sphere.txt\" in this world's storage.",
                RCSVisualizer.CalculateSphere),

            ["render"] = new Command(
                "DetEq",
                "render <rcs | vcs | irs | reset> <scalemult> <points if you want points, otherwise leave empty>. Renders a given detection value sphere from 'getallangles'. Value (<type>) must be specified as an argument, as either 'RCS', 'VCS', 'IRS', or 'reset' if you wish to stop rendering. 1m away from grid center corresponds to 10m^2 RCS/VCS, or 100000Wm^2 for IRS by default, or corresponds to the given scale value if <scalemult> is specified.",
                RCSVisualizer.RenderSphere),
